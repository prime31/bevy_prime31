...
pub struct CameraRig {
    ...
    pub final_transform: CameraTransform,
}
...
impl CameraRig {
    ...
    fn update(&mut self, delta_time_seconds: f32) {
        let mut parent_transform = CameraTransform::IDENTITY;
    ...
    /// Runs all the drivers in sequence, animating the rig, and producing a final transform of the camera
    /// which is copied into the passed in Transform.
    ///
    /// Camera rigs are approximately framerate independent, so `update` can be called at any frequency.
    pub fn update_into(&mut self, delta_time_seconds: f32, transform: &mut Transform) {
        self.update(delta_time_seconds);
        transform.translation = self.final_transform.position;
        transform.rotation = self.final_transform.rotation;
    }
...
impl CameraRigBuilder {
    ...
    pub fn build(self) -> CameraRig {
        let mut rig = CameraRig {
            ...
            final_transform: CameraTransform::IDENTITY,
        };
